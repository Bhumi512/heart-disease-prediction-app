...
chest_pain = st.selectbox('Chest Pain Type(1 - Typical Angina, 2 - Atypical Angina, 3 - Non-Anginal Pain, 4 - Asymptomatic)', options=[1, 2, 3, 4])
...
exercise_angina = st.selectbox('Exercise Induced Angina(0 - no chest pain during exercise, 1 - experience chest pain during exercise)', options=[0, 1])
st_depression = st.number_input('ST Depression', min_value=0.0, max_value=10.0, value=0.0)
slope_st = st.selectbox('Slope of the peak exercise ST segment (1 - upward, 2 - Flat, 3 - Dowmward)', options=[1, 2, 3])
num_vessels = st.selectbox('Number of Major Vessels deffected by blockage (0-3)', options=[0, 1, 2, 3])
